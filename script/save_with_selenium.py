...
index = 0
index = contenu_web.find(pattern, index+1)
while index >= 0:
    min_index = index
    while contenu_web[min_index] != '{':
        min_index = min_index - 1
    max_index  = index + 1
    nb_open = 1
    print(f'EXTRACT = {contenu_web[index-25:index+50]}')
    while nb_open > 0:
        if contenu_web[max_index] == '}':
            nb_open -= 1
        if contenu_web[max_index] == '{':
            nb_open += 1
        max_index = max_index + 1
    print(contenu_web[min_index:max_index+1])
    index = contenu_web.find(pattern, index+1)
